...
bool isLegalPosition(int n, int row, int col)
{
    int i, j;
...
bool nextLegalPosition(int n, int col)
{
    if (col == n)
    {
        printBoard(n);
    ...
    for (int i = 0; i < n; i++)
    {
        if ( isLegalPosition(n, i, col) )
        {

            board[i][col] = 1;

            res = nextLegalPosition(n, col + 1) || res;
...
void backtrackAll(int n)
{
    ...
    if (nextLegalPosition(n, 0) == false)
    {
